...
float advectLevelSet(){
    ...
    // y:
    // z:  (zSlice)/32
    float thisPoint = texture(levelSetTexture, vec3(TextureCoord, float(zSlice)*step)).x;
    ...
    //int slice = 22;
    vec4 vel2 = texture(velocityTexture, vec3(TextureCoord, float(zSlice)*step)) ;/// vec3(1.0f, 1.0f, step);
    /*
    if (vel2.x > 0.0f){
        return -1.0f;
    }
    ...
    vec3 vel = vel2.xyz;
    vel /= 1e50;
    vel = vec3(0,0,0); // why does it still disappear with velocity == 0? Also, why does A component of frag color make a difference?
    //vel = vec3(1e10,0,0);
   ...
   // vel = vec3(0,0,-1.0f);
    ...
    return float(texture(levelSetTexture, vec3(TextureCoord, float(zSlice)*step) - vel * timeStep).x);
}

void main(){
    
    //newLevelSet = -10.0f; return;
    newLevelSet= advectLevelSet();
    //FragColor = vec4(temp, 0.0f, 0.0f, 1.0f); return;
